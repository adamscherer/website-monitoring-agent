...
class WebsiteAgent:
    ...
    def _update_baseline_metrics(self):
        ...
        response_times = [float(m['response_time'].replace('s', '')) for m in self.performance_history]
        content_lengths = [m['content_length'] for m in self.performance_history]
        
        self.baseline_metrics = {
            'avg_response_time': sum(response_times) / len(response_times),
            'std_response_time': (sum((x - (sum(response_times) / len(response_times))) ** 2 for x in response_times) / len(response_times)) ** 0.5,
            'avg_content_length': sum(content_lengths) / len(content_lengths),
            'typical_status_code': max(set(m['status_code'] for m in self.performance_history), key=self.performance_history.count)
        }
    ...
    def _make_decisions(self, metrics):
        ...
        if self.baseline_metrics:
            ...
            if current_response_time > self.baseline_metrics['avg_response_time'] + 2 * self.baseline_metrics['std_response_time']:
                decisions.append({
            ...
            content_diff_percent = abs(metrics['content_length'] - self.baseline_metrics['avg_content_length']) / self.baseline_metrics['avg_content_length'] * 100
        ...
        for decision in decisions:
            ...
            if decision['type'] == 'adjust_monitoring':
                if decision['action'] == 'decrease_interval':
                    new_interval = max(0.5, self.current_check_interval * 0.5)  # Don't go below 30 seconds
    ...
    def run(self):
        """Start the agent"""
        self.logger.info(f"Starting Website Management Agent with config: {self.config['website']['check_interval']}")
